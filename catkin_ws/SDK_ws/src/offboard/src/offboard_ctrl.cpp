...
void position_control() {
    ...
    if(state == 0) {
        ...
        // land_cmd.request.altitude = vicon_pose.transform.translation.z; // Use current vicon z
        // land_cmd.request.latitude = 0; // 0 because not using gps
        // land_cmd.request.longitude = 0; // 0 because not using gps
        // land_cmd.request.min_pitch = 0; // minimum pitch angle for the drone. min roll is not applicable in this service
        // land_cmd.request.yaw = 0;


        // ROS_INFO("check landing position: alt:%.2f, lat:%.2f, longi:%.2f", land_cmd.request.altitude, land_cmd.request.latitude, land_cmd.request.longitude);
        // if(set_mode_client.call(land_cmd) && land_cmd.response.success) {
        //     ROS_INFO("Landing initiated.");
        // }
    }
    else if(state == 1) { //take off
        target = Eigen::Vector3d(vicon_posi_x, vicon_posi_y, vicon_posi_z + 0.5);
    }
    else if(state == 2) {
        target = Eigen::Vector3d(vicon_posi_x, vicon_posi_y + 0.5, vicon_posi_z + 0.5);
    }
    else if(state == 3) {
        target = Eigen::Vector3d(vicon_posi_x + 0.5, vicon_posi_y + 0.5, vicon_posi_z + 0.5);
    }
    else if(state == 4) {
        target = Eigen::Vector3d(vicon_posi_x + 0.5, vicon_posi_y, vicon_posi_z + 0.5);
    }
    else if(state == 5) {
        target = Eigen::Vector3d(vicon_posi_x, vicon_posi_y, vicon_posi_z + 0.5);
    }
    ...
    // mavros_msgs::PositionTarget raw_target_pose;
    // raw_target_pose.header.stamp = ros::Time::now();
    // raw_target_pose.header.frame_id = "map";

    // raw_target_pose.coordinate_frame = mavros_msgs::PositionTarget::FRAME_LOCAL_NED; // Set the correct coordinate frame
    // raw_target_pose.type_mask = mavros_msgs::PositionTarget::IGNORE_VX |
    //                             mavros_msgs::PositionTarget::IGNORE_VY |
    //                             mavros_msgs::PositionTarget::IGNORE_VZ |
    //                             mavros_msgs::PositionTarget::IGNORE_AFX |
    //                             mavros_msgs::PositionTarget::IGNORE_AFY |
    //                             mavros_msgs::PositionTarget::IGNORE_AFZ |
    //                             mavros_msgs::PositionTarget::IGNORE_YAW_RATE; // Mask to ignore fields you are not setting

    // raw_target_pose.position.x = target[0];
    // raw_target_pose.position.y = target[1];
    // raw_target_pose.position.z = target[2];
    // raw_target_pose.yaw = 0;

    // pos_raw_pub.publish(raw_target_pose);
    ...
    if(isAtPosition(target[0], target[1], target[2], 0.2, 0.1)) {
        if(state >= num_of_state_to_run) {
            ...
            state++;
            ROS_INFO_ONCE("Position x:%.2f, y:%.2f, z:%.2f reached. Moving to next desired position. State is:%.2d", target[0], target[1], target[2], state);            
        }
    }
