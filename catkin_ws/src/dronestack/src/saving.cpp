...
private:
    ...
    void gazebo_cb(const gazebo_msgs::ModelStates::ConstPtr& msg) {
        ...
        if (msg->name.size() > 1 && msg->pose.size() > 1 && msg->twist.size() > 1) {
            ...
            // publish position and velocity to vision pose
    ...
    void initialize_setpoint() {
        ...
        // // transfer the datatype of gazebo position, so that i can publish it to vision pose
        // gazebo_position.pose.position.x = position_point.x;
        // gazebo_position.pose.position.y = position_point.y;
        // gazebo_position.pose.position.z = position_point.z;
    ...
    void main_loop() {
        ...
        while(ros::ok()) {
            ...
            // check if the drone reaches the target position
